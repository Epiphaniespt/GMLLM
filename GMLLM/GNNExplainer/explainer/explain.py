...
class Explainer:
    def __init__(
        ...
        self.NODE_VIS_THRESHOLD = 5
        self.EDGE_VIS_THRESHOLD = 5

    def explain(
        self, node_idx, graph_idx=0, graph_mode=False, unconstrained=False, model="exp",label_to_explain=None
    ):
        ...
        current_sub_edges = torch.sum(sub_adj_tensor[0] > 0).item()
        ...
        if self.graph_mode:
            ...
            print("Explaining label: ", pred_label)
        else:
        ...
        explainer = ExplainModule(
            ...
            ###
            is_small_graph=is_current_graph_small,
        )
        if self.args.gpu:
        ...
        if model == "grad":
            explainer.zero_grad()
            ypred_dummy, _ = explainer(node_idx_new, unconstrained=unconstrained)
            epoch = explainer.epoch
...
class ExplainModule(nn.Module):
    def __init__(
        ...
        self.graph_mode = graph_mode
        self.is_small_graph = is_small_graph
    ...
    def _masked_adj(self):
        ...
        self.current_masked_adj = masked_adj * self.diag_mask
    ...
    def forward(self, node_idx, unconstrained=False, mask_features=True, marginalize=False):
        ...
        if self.current_masked_adj.requires_grad:
            self.current_masked_adj.retain_grad()
        if x.requires_grad:
            x.retain_grad()
    ...
    def loss(self, pred, pred_label, node_idx, epoch):
        ...
        if self.graph_mode:
            # For graph classification, get the probability of the target class.
            logit = pred[pred_label]
        else:
            # For node classification, get the probability for the specific node.
            logit = pred[node_idx, pred_label] # Assumes node-level logic
        ...
        # laplacian
        D = torch.diag(torch.sum(self.current_masked_adj[0], 0))
        m_adj = self.current_masked_adj if self.graph_mode else self.current_masked_adj[self.graph_idx]
        L = D - m_adj
    ...
    def log_masked_adj(self, node_idx, epoch, name="mask/graph", label=None):
        ...
        if self.graph_mode:
            ...
            G = io_utils.denoise_graph(
                masked_adj, node_idx, feat=self.x[0].cpu().detach().numpy(),
                threshold=vis_threshold,
                max_component=True
            )
            io_utils.log_graph(
